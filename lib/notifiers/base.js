...
class BaseNotifier {
	...
	handleConfig(config, setting, value) {
		...
		if(!this.registerVariables.hasOwnProperty(setting)) {
			return {
				success: false,
				messages: [
		...
		const variable = this.registerVariables[setting];
		...
		config.services[this.name][setting] = parsedValue;

		const messages = [F.SUCCESS(`${this.name} setting ${C.CYAN}${setting}${C.RESET} updated`)];
